...
class Command:
    ...
    def calculate_length(self):
        ...
        """
        return (
            self.HEADER_LENGTH + self.LENGTH_LENGTH +
            self.COMMAND_LENGTH + len(self.bytes) +
            self.FOOTER_LENGTH + self.CHECK_LENGTH
        )
    ...
    def _encode_packet(self):
        ...
        return (
            ...
            self.convert_bytes() + self.FRAME_FOOTER
        )
...
class SetBaudrate(Command):
    ...
    def __init__(self, baud):
        super().__init__(self.COMMAND, struct.pack('>L', baud))
...
class SetStorageMode(Command):
    ...
    def __init__(self, target=None):
        super().__init__(self.COMMAND, data=[target or self.NAND_MODE])
...
class SetCurrentDisplayRotation(Command):
    ...
    def __init__(self, rotation=None):
        super().__init__(self.COMMAND, rotation or self.NORMAL)
...
class SetPallet(Command):
    ...
    def __init__(self, fg=None, bg=None):
        fg = fg or self.BLACK
        bg = bg or self.WHITE
        super().__init__(self.COMMAND, [fg, bg])
...
class SetFontSize(Command):
    ...
    def __init__(self, command, size=None):
        super().__init__(command, [size or self.THIRTYTWO])
...
class SetEnFontSize(SetFontSize):
    ...
    def __init__(self, size=None):
        super().__init__(self.COMMAND, size or self.THIRTYTWO)
...
class DisplayText(Command):
    ...
    def __init__(self, x, y, text):
        data = struct.pack(">HH", x, y) + text + b'\x00'
...
class DrawCircle(Command):
    ...
    def __init__(self, x, y, radius):
        data = struct.pack(">HHH", x, y, radius)
        super().__init__(self.COMMAND, data)
...
class DrawTriangle(Command):
    ...
    def __init__(self, x1, y1, x2, y2, x3, y3):
        data = struct.pack(">HHHHHH", x1, y1, x2, y2, x3, y3)
        super().__init__(self.COMMAND, data)
...
class DrawRectangle(Command):
    ...
    def __init__(self, x1, y1, x2, y2):
        data = struct.pack(">HHHH", x1, y1, x2, y2)
        super().__init__(self.COMMAND, data)
...
class EPaper:
    ...
    def __init__(self, port=None, reset=None, wakeup=None, mode=None):
        """
        ...
        @param mode The mode of GPIO pin addressing (default is GPIO.BOARD).
        """
        self.serial = serial.Serial(port or self.PORT_DEVICE)
        self.serial.baudrate = 115200  # default for device
        self.serial.bytesize = serial.EIGHTBITS
        self.serial.parity = serial.PARITY_NONE

        GPIO.setmode(mode or GPIO.BOARD)
        GPIO.setup(reset or self.PIN_RESET, GPIO.OUT)
        GPIO.setup(wakeup or self.PIN_WAKEUP, GPIO.OUT)
    ...
    def send(self, command):
        ...
        if self.bytes_expected >= self.RESPONSE_READ_THRESHOLD:
            self.read_responses()
    ...
    def read(self, size=100, timeout=5):
        ...
        print(
            "read took %0.2f seconds. expected: %d got: %d data: %s"
    ...
    def read_responses(self, timeout=3):
        ...
        print(
            "read: %d, read time: %0.2f"
            % (len(b), time.time() - start_time)
        )
